...
// COMMANDS---------------------------------
uint8_t read_command[8]={default_addr,0x04,0x00,0x00,0x00,0x0A,0x64,0x64};
...
uint16_t CRC_16( const unsigned char *buf, unsigned int len )
{
...
energyValues_t read_PZEM()
{
	...
	uint16_t crc_buffer = buf[24]<<8 |buf[23]; // gets CRC from the lasts two bytes
	...
	if(crc_calculated==crc_buffer)
	{
		...
		values.power=((uint32_t) buf[9]<<8 	| (uint32_t)buf[10] 	| (uint32_t)buf[11]<<24 	|(uint32_t)buf[12]<<16);
		values.energy= ((uint32_t) buf[13]<<8 	| (uint32_t)buf[14] | (uint32_t)buf[15]<<24 	|(uint32_t)buf[16]<<16);
		values.frequency= ((uint32_t)buf[17]<<8 | (uint32_t)buf[18]);
		values.power_factor=((uint32_t)buf[19]<< 8  	|(uint32_t) buf[20]);
		values.alarm_status=((uint32_t)buf[21]<< 8  	|(uint32_t) buf[22]);
		//---------------------------------------------------------------
	}
